...
typedef struct {
  ...
  uint8_t received; // was a message successfully received?
} ws_header_t;
...
typedef struct {
  ...
  uint8_t ping;            // did we send a ping?
...
uint8_t ws_is_connected(ws_client_t client); // returns 1 if connected, status updates after send/read/connect/disconnect
int ws_send(ws_client_t* client,WEBSOCKET_OPCODES_t opcode,char* msg,uint64_t len,uint8_t mask); // sends message. this function performs the masking
