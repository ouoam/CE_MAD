...
uint8_t ws_is_connected(ws_client_t client) {
  if(client.conn)
    return 1;
  return 0;
}
...
static void ws_generate_mask(ws_header_t* header) {
  ...
  header->key.full = 0; // generate a random 32 bit number
}
...
int ws_send(ws_client_t* client,WEBSOCKET_OPCODES_t opcode,char* msg,uint64_t len,uint8_t mask) {
  char* out;
  char* encrypt;
  uint64_t pos;
  uint64_t true_len;
